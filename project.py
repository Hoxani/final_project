...
class Hangman:
    def __init__(self, root) -> None:
        ...
        self.header = tk.Label(root, text="WELCOME TO HANGMAN")
        self.header.pack(padx=10, pady=10)

        self.progress_bar = tk.Label(root, text=f"I am thinking of a word that is {len(self.secret_word)} long.", foreground="blue")
        self.progress_bar.pack(padx=10, pady=10)

        self.feedback = tk.Label(root, text="Please guess a letter.")
        ...
        self.start = tk.Button(root, text="Enter", command=self.game_progress)
    ...
    def game_progress(self):
        ...
        if self.has_player_won():
            ...
            messagebox.showinfo("game", f"🎉 Correct!, your score is {self.score}.")
            self.reset_game()
        elif self.guesses <= 0:
            messagebox.showinfo("game", f"You have ran out of guesses, the sceret word was {self.secret_word}.")                  
            self.reset_game()
